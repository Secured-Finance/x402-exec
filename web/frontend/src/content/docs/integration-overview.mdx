...
Use `@secured-finance/core` to fill the required fields consistently.

```payment-requirements.ts
import { addSettlementExtra, getNetworkConfig, TransferHook } from "@secured-finance/core";
...
You can use `@secured-finance/fetch` to handle both modes automatically.
...
Or roll your own using `@secured-finance/core/calculateCommitment` and EIP‑712.
