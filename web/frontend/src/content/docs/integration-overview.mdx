...
<CodeTabs
  codes={{
    npm: "npm install x402@npm:@x402x/x402@0.6.6-patch.3",
    pnpm: "pnpm add x402@npm:@x402x/x402@0.6.6-patch.3",
    yarn: "yarn add x402@npm:@x402x/x402@0.6.6-patch.3",
    bun: "bun add x402@npm:@x402x/x402@0.6.6-patch.3",
  }}
  lang="bash"
/>
...
Use `@secured-finance/x402-core` to fill the required fields consistently.

```payment-requirements.ts
import { addSettlementExtra, getNetworkConfig, TransferHook } from "@secured-finance/x402-core";
...
const withSettlement = addSettlementExtra(base, {
  ...
  payTo: "0xMerchantAddress...", // Final recipient (merchant) address
});
```

**Note:** The `payTo` parameter in `addSettlementExtra` specifies the final recipient (merchant) address.
This is different from the base `requirements.payTo` which points to the SettlementRouter for the EIP-3009 authorization.
The SettlementRouter will use the `extra.payTo` value to determine where funds should ultimately go.
...
For more information about facilitator fees, how they're calculated, and how to work with them, see the [Facilitator Fee documentation](./facilitator-fee.mdx).
...
You can use `@secured-finance/x402-fetch` to handle both modes automatically.
...
Or roll your own using `@secured-finance/x402-core/calculateCommitment` and EIP‑712.
