...
Use `@sf-x402/core` to fill the required fields consistently.

```payment-requirements.ts
import { addSettlementExtra, getNetworkConfig, TransferHook } from "@sf-x402/core";
...
You can use `@sf-x402/fetch` to handle both modes automatically.
...
Or roll your own using `@sf-x402/core/calculateCommitment` and EIP‑712.
