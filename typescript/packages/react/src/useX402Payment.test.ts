...
vi.mock("@secured-finance/x402-fetch", () => ({
...
describe("useX402Payment", () => {
  ...
  describe("pay method", () => {
    ...
    it("should throw error when no wallet client available", async () => {
      ...
      await expect(result.current.pay("/api/test")).rejects.toThrow("No wallet client available");

      await waitFor(() => {
        expect(result.current.status).toBe("error");
        expect(result.current.error).toBe(
          "No wallet client available. Make sure wallet is connected.",
        );
      });
    });
