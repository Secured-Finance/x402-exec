...
contract SettlementRouterTest is Test {
    ...
    function testSettleAndExecuteWithSimpleHook() public {
        ...
        bytes32 contextKey = router.calculateContextKey(
            payer,
            address(token),
            nonce
        );
    ...
    function testTransferHookDistributedMode() public {
        ...
        splits[0] = TransferHook.Split({recipient: merchant, bips: 7000});
        splits[1] = TransferHook.Split({recipient: platform, bips: 3000});
    ...
    function testInvalidTransferSplit() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                address(transferHook),
                abi.encodeWithSelector(
                    TransferHook.InvalidTotalBips.selector,
                    11000
                )
            )
        );
    ...
    function testFacilitatorFeeAccumulation() public {
        ...
        assertEq(
            router.getPendingFees(facilitator, address(token)),
            facilitatorFee
        );
    ...
    function testClaimFeesMultipleTokens() public {
        ...
        assertEq(
            router.getPendingFees(facilitator, address(token2)),
            facilitatorFee
        );
        ...
        assertEq(
            token2.balanceOf(facilitator),
            balanceBefore2 + facilitatorFee
        );
    ...
    function testHookReceivesAllParameters() public {
        ...
        bytes32 expectedContextKey = router.calculateContextKey(
            payer,
            address(token),
            nonce
        );
    ...
    function testSaltUniquenessEnsuresIdempotency() public {
        ...
        bytes32 contextKey1 = router.calculateContextKey(
            payer,
            address(token),
            nonce1
        );
        ...
        bytes32 contextKey2 = router.calculateContextKey(
            payer,
            address(token),
            nonce2
        );
    ...
    function testRecoveryAfterDirectTransfer() public {
        ...
        assertEq(
            router.getPendingFees(address(this), address(token)),
            facilitatorFee
        );
    }
