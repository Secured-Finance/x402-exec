...
if (require.main === module) {
	...
	if (missing.length > 0) {
		...
		console.error("\nUsage:");
		console.error("  tsx script/encodeSwapConfig.ts \\");
		console.error("    --dexAggregator <address> \\");
		console.error("    --swapCalldata <hex> \\");
		console.error("    --toToken <address> \\");
		console.error("    --minAmountOut <number> \\");
		console.error("    --isNativeToken <true|false>");
		process.exit(1);
	}

	try {
		const config: SwapConfigParams = {
			dexAggregator: params.dexAggregator as Address,
			swapCalldata: params.swapCalldata as Hex,
			toToken: params.toToken as Address,
			minAmountOut: params.minAmountOut,
			isNativeToken: params.isNativeToken === "true",
		};

		const encoded = encodeSwapConfig(config);

		console.log("Encoded SwapConfig data:");
		console.log(encoded);
		console.log("\nParameters:");
		console.log("  dexAggregator:", config.dexAggregator);
		console.log("  swapCalldata:", config.swapCalldata);
		console.log("  toToken:", config.toToken);
		console.log("  minAmountOut:", config.minAmountOut);
		console.log("  isNativeToken:", config.isNativeToken);
	} catch (error) {
		console.error(
			"Error encoding SwapConfig:",
