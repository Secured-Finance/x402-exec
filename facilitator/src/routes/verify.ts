...
import { getSupportedNetworks, getNetworkConfig } from "@secured-finance/core";
...
export function createVerifyRoutes(
  ...
  router.post("/verify", ...(middlewares as any), async (req: Request, res: Response) => {
    try {
      ...
      // For custom networks (sepolia, filecoin-calibration), use @secured-finance/core config
      let chain;
      try {
        chain = evm.getChainFromNetwork(paymentRequirements.network);
      } catch (error) {
        // If x402 doesn't know about this network, it's a custom one
        // Get chain config from @secured-finance/core and construct viem chain
